...
# Тесты. Закоментировать чтобы играть
cell = Cell()
...
assert hasattr(TicTacToe, 'show') and hasattr(TicTacToe, 'human_go') and hasattr(TicTacToe,
                                                                                 'computer_go'), "класс TicTacToe должен" \
                                                                                                 " иметь методы show, " \
                                                                                                 "human_go, computer_go"
...
game.init()
assert game.is_human_win == False and game.is_computer_win == False and game.is_draw == False, "при инициализации игры " \
                                                                                               "атрибуты is_human_win, " \
                                                                                               "is_computer_win, is_draw" \
                                                                                               " должны быть равны " \
                                                                                               "False, возможно не " \
                                                                                               "пересчитывается статус " \
                                                                                               "игры при вызове метода " \
                                                                                               "init()"

game[0, 0] = TicTacToe.HUMAN_X
game[1, 1] = TicTacToe.HUMAN_X
game[2, 2] = TicTacToe.HUMAN_X
assert game.is_human_win and game.is_computer_win == False and game.is_draw == False, "некорректно пересчитываются " \
                                                                                      "атрибуты is_human_win, " \
                                                                                      "is_computer_win, is_draw. " \
                                                                                      "Возможно не пересчитывается " \
                                                                                      "статус игры в момент присвоения " \
                                                                                      "новых значения по индексам: " \
                                                                                      "game[i, j] = value"
...
assert game.is_human_win == False and game.is_computer_win and game.is_draw == False, "некорректно пересчитываются " \
                                                                                      "атрибуты is_human_win, " \
                                                                                      "is_computer_win, is_draw. " \
                                                                                      "Возможно не пересчитывается " \
                                                                                      "статус игры в момент присвоения " \
...
# Раскомментировать чтобы играть
# game = TicTacToe()
# game.init()
# step_game = 0
# while game:
